...
def backdoor_model_trainer(
    ...
    train_process = []
    print(
        "### target label is %d, EPOCH is %d, Learning Rate is %f"
        % (trigger_label, epoch, lr)
    )
    print(
        "### Train set size is %d, ori test set size is %d, tri test set size is %d\n"
        % (
            len(train_data_loader.dataset),
            len(test_data_ori_loader.dataset),
            len(test_data_tri_loader.dataset),
        )
    )
    for epo in range(epoch):
        ...
        print(
            "# EPOCH%d   loss: %.4f  training acc: %.4f, ori testing acc: %.4f, trigger testing acc: %.4f\n"
        ...
        df.to_csv(
            "./logs/%s_train_process_trigger%d.csv" % (dataname, trigger_label),
            index=False,
            encoding="utf-8",
        )
    ...
    if print_perform and mode is not "backdoor":
        print(
            classification_report(
                y_true.cpu(), y_predict.cpu(), target_names=data_loader.dataset.classes
            )
        )
